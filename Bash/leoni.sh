...
# estrai anni
<./raw.json jq '[.[].children | select(.|length > 2)] | {anno:.[][0].children[0].text}' \
| mlr --j2c cat >./anni.csv
# estrai titoli per anni con un solo vincitore
<./raw.json jq  '[.[].children | select(.|length > 2)] | {titoloUno:.[][1].children[0].children[0].text}' \
| mlr --j2c cat >./titolo1.csv
# estrai titoli per anni con più di un vincitore
<./raw.json jq '[.[].children | select(.|length > 2)] | {titoloDue:.[][0].children[0].children[0].text}' \
| mlr --j2c cat >./titolo2.csv
# estrai regista per anni con un solo vincitore
<./raw.json jq '[.[].children | select(.|length > 2)] | {registaUno:.[][2].children[0].text}' \
| mlr --j2c cat >./regista1.csv
# estrai regista per anni con più di un vincitore
<./raw.json jq '[.[].children | select(.|length > 2)] | {registaDue:.[][1].children[0].text}' \
...
<./raw.json jq '[.[].children | select(.|length > 2)] | .[] | if (.|length) > 3 then 
